...
function addon:OnCommReceived(prefix, message, _, sender)
    ...
    if self.db.profile.debug then
        self:PrintMessage("OnCommReceived: %s; Sender: %s; Data: %s", command, sender, data)
    end
    ...
    elseif command == 'LEAD' then
        ...
        if self.db.profile.debug or addon.Version == 'v9.9.9' then self:PrintMessage("Lead taken by " .. name) end
    elseif command == 'JOIN' then
        local name, intVersion = strsplit(",", data)

        self:CheckRelease(intVersion, name)

        self:SendCommMessage(addon._commPrefix, fmt('LEAD|%s,%d', self.cluster.lead, addon.release.int), "WHISPER",
                             sender)
    elseif command == 'SYNC' then
        if self.db.profile.announcedSpells[data] ~= true then self.db.profile.announcedSpells[data] = true end
    else
        if self.session.UnsupportedComm[command] == nil then
            self.session.UnsupportedComm[command] = true
            self:PrintMessage(self.L["Broadcast"].Unrecognized, command, sender)
        end
    end
end

function addon:Broadcast(command, data)
    if not self.db.profile.enable or UnitInBattleground("player") ~= nil then return end

    if self.db.profile.debug then self:PrintMessage("Broadcasting %s: %s", command, data) end

    self:SendCommMessage(addon._commPrefix, fmt("%s|%s", command, data), "RAID")
end

function addon:PrintLead() self:PrintMessage(self.L["Cluster"].Lead, self.cluster.lead) end
...
function addon:BroadcastLead(playerName)
    if not self.db.profile.enable or not self.db.profile.whisper or not playerName or UnitInBattleground("player") ~=
        nil or not self.db.profile.isLatestVersion then return end
...
function addon:SyncBroadcast(array, index)
    ...
    if array == nil or index == nil then
        self:PrintMessage(self.L["Cluster"].Sync, self:CountCache(self.db.profile.announcedSpells))

        local ordered_announcements = {}
        for k in pairs(self.db.profile.announcedSpells) do tinsert(ordered_announcements, k) end
        ...
        for i = index, index + batch_size do
            ...
            if self.db.profile.debug then print(fmt("Sending %d - %s", i, array[i])) end

            self:SendCommMessage(addon._commPrefix, fmt("%s|%s", 'SYNC', array[i]), "RAID", "BULK")
        end

        C_Timer.After(3, function() self:SyncBroadcast(array, index + batch_size) end)
    end
end
...
function addon:CheckRelease(theirIntRelease, name)
    ...
    if theirIntRelease == 999 or addon.Version == 'v9.9.9' or not theirIntRelease then return end
