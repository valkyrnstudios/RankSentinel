...
function RankSentinel:OnCommReceived(prefix, message, distribution, sender)
    if prefix ~= RankSentinel._commPrefix or sender == self.playerName then return end
...
function RankSentinel:ClusterBroadcast(command, data)
    self:SendCommMessage(RankSentinel._commPrefix, string.format("%s|%s", command, data),
                         "RAID")
end
